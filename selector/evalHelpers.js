...
function getAttributesFromElems(maxLevelsString) {
  function isBlank(o){
  ...
  function getAttributesOfElemAndAncestors(elem, maxLevels) {
    function getElemAndAncestorsList(elem, maxLevels){
      var elemAndAncestors = [];
      var count = 0;
      while (elem !== undefined && elem !== null && elem.nodeName !== 'HTML' && count < maxLevels) {
    ...
    var elemAndAncestors = getElemAndAncestorsList(elem, maxLevels);
    var attributesOfElemAndAncestors = getAttributes(elemAndAncestors);

    return attributesOfElemAndAncestors;
  }

  attributesAndFrames.attributes = getAttributesOfElemAndAncestors(lastSelectedElem, parseInt(maxLevelsString));
